...
// TODO: Swap the enumeration system with a modular function linked list
enum FLAGS
{
	HEADERS = 1,
	VISUAL = 2,
...
void FPop(int Layers, int* Neuron_Count, double** Neuron, double** Weights, int Flags)
{
	// Forward Propogate
	// Matrix Multiply: N2 = Sigmoid{ W*N1 }
	if(Flags | HEADERS){cout << "Forward Propogate\n";}
...
void FPop(int Layers, int* Neuron_Count, double** Neuron, double** Weights)
{
	FPop(Layers,Neuron_Count,Neuron,Weights,0);
}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	for(int i=0;i<Layers-1;++i)
	{
		...
		{
			for(int src=0;src<Neuron_Count[i];++src) // Matrix multiply
			{
				Weights[i][dst*Neuron_Count[i]+src] = (xorshfdbl()*2.0)-1.0;
				cout << Weights[i][dst*Neuron_Count[i]+src] << " ";
			}
			cout << "\n";
		}
		cout << "\n";
	...
	// Derivative setup
	memset(dDen[Layers-1],1.0,Neuron_Count[Layers-1]*sizeof(double));
	memcpy(dNum[Layers-1],Neuron[Layers-1],Neuron_Count[Layers-1]*sizeof(double));
	for(int i=0;i<Neuron_Count[Layers-1];++i) // Discrete derivative for the numerator
		dNum[Layers-1][i] -= Expected[0][i];

	// Backward Propogate
	for(int layer=Layers-1;0<layer;--layer) // BACKpropogation, doesn't effect the first layer
	...
	for(int layer=0;layer<Layers-1;++layer)
		for(int dst=0;dst<Neuron_Count[layer+1];++dst)
		{
			...
			{
				int pos = dst*Neuron_Count[layer]+src;
				Weights[layer][pos] -= 0.1*Numer[layer][pos]/Denom[layer][pos];
